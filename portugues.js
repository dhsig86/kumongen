...
(function() {
    ...
    function loadSavedState() {
        ...
        if (!storage) return;
        const savedLevelId = storage.getItem('kumongen_por_level');
        if (savedLevelId) {
            currentLevelId = savedLevelId;
        }

        const savedParams = storage.getItem('kumongen_por_params');
        if (savedParams) {
            try {
    ...
    function saveState() {
        ...
        if (!storage) return;
        try {
            storage.setItem('kumongen_por_level', currentLevelId);
            storage.setItem('kumongen_por_params', JSON.stringify(customParams));
        } catch (e) {}
    }
    ...
    const DEFAULT_WORDS_P4 = [
        ...
        { word: 'PICOLÉ', parts: ['PI','CO','LÉ'] },
        { word: 'CORUJA', parts: ['CO','RU','JA'] },
        { word: 'BEBIDA', parts: ['BE','BI','DA'] },
        { word: 'NOVELA', parts: ['NO','VE','LA'] },
        { word: 'VACINA', parts: ['VA','CI','NA'] },
        { word: 'APITO', parts: ['A','PI','TO'] },
        { word: 'TAPETE', parts: ['TA','PE','TE'] },
        { word: 'AMIGO', parts: ['A','MI','GO'] },
        { word: 'AMIGA', parts: ['A','MI','GA'] },
        { word: 'BUZINA', parts: ['BU','ZI','NA'] },
        { word: 'PIRATA', parts: ['PI','RA','TA'] },
        { word: 'GAVETA', parts: ['GA','VE','TA'] },
        { word: 'TIJOLO', parts: ['TI','JO','LO'] },
        { word: 'CAMELO', parts: ['CA','ME','LO'] },
        { word: 'BIGODE', parts: ['BI','GO','DE'] },
        { word: 'TELEFONE', parts: ['TE','LE','FO','NE'] },
        { word: 'ABACAXI', parts: ['A','BA','CA','XI'] },
        { word: 'LIMONADA', parts: ['LI','MO','NA','DA'] },
        { word: 'SABONETE', parts: ['SA','BO','NE','TE'] },
        { word: 'CAPACETE', parts: ['CA','PA','CE','TE'] },
        { word: 'MARACUJÁ', parts: ['MA','RA','CU','JÁ'] }
    ];
    ...
    window.selectLevel = function(id) {
        ...
        if (id === 'p3' && previousLevel !== 'p3') {
            ...
            customParams.wordList = DEFAULT_WORDS_P4.map(item => ({
                word: item.word,
                parts: [...item.parts]
            }));
        }
